...
public:
  ...
  static Result success(const T &value) { return Result(SuccessTag{}, value); }

  // Create error result with error
  static Result error(const E &error) { return Result(ErrorTag{}, error); }

  // Check if result is successful
  bool is_success() const { return success_value.has_value(); }

  // Check if result is error
  bool is_error() const { return error_value.has_value(); }

  // Get success value (returns std::nullopt if error)
  std::optional<T> get_success() const { return success_value; }

  // Get error value (returns std::nullopt if success)
  std::optional<E> get_error() const { return error_value; }
